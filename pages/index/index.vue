		...
		methods: {
			...
			getList(){
				...
				.then(res=>{
					...
					if (this.newsList[index].list) {
						this.newsList[index].list.forEach( function(item, index) {
							if (item.userpic.indexOf(host) == -1) {
								item.userpic = host + item.userpic
							}
							if (item.titlepic.indexOf(host) == -1) {
								item.titlepic = host + item.titlepic
							}
						})
					}
					console.log('this.newsList', this.newsList[index])
					if (isrefresh) {
