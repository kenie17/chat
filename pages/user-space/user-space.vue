	...
	<view>
		...
		<uni-popup ref="popup" type="top">
			<view class="flex align-center justify-center font-md border-bottom py-2" hover-class="bg-light" @click="doBlack">
				<text class="iconfont icon-sousuo1 mr-2"></text> 
			...
			<view v-if="!userinfo.isblack" class="flex align-center justify-center font-md py-2" hover-class="bg-light" @click="openChat">
				<text class="iconfont icon-chat mr-2"></text> 聊天
			</view>
		</uni-popup>
		...
		methods: {
			...
			getUserInfo(){
				this.$H.post('/getuserinfo',{
					...
					if (this.userinfo.userpic.indexOf(host) == -1) {
						this.userinfo.userpic = host + this.userinfo.userpic
					}
					console.log('userpic', this.userinfo.userpic)
