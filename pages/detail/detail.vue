	...
	<view>
		...
		<view class="px-2">
			...
			:key="index">
				...
				<!-- uni-comment-face -->
					<view class="">
						<image :src="item.userpic" style="width: 100rpx;height: 100rpx;margin-right: 20rpx;" class="rounded-circle"></image>
					</view>
		...
		<!-- http://localhost:8080/#/pages/detail/detail?detail=%7B%22id%22%3A338,%22user_id%22%3A392,%22username%22%3A%2215218378694%22,%22userpic%22%3A%22http%3A%2F%2Fchat.cloud-cc.top%2Fpics%2F044.jpg%22,%22newstime%22%3A1580832550,%22title%22%3A%22789%22,%22titlepic%22%3A%22%22,%22isFollow%22%3Afalse,%22support%22%3A%7B%22type%22%3A%22%22,%22support_count%22%3A1,%22unsupport_count%22%3A0%7D,%22comment_count%22%3A1,%22share_num%22%3A0%7D -->
		<!-- 占位 -->
		...
		methods: {
			__init(data){
				...
				this.$H.get('/post/'+this.info.id).then(res=>{
					...
					let host = 'http://chat.cloud-cc.top/';
					if (this.images) {
						this.images.forEach(function(item, index) {
							if (item.url.indexOf(host) == -1) {
			...
			__ArrSort(arr,id = 0){
				...
				var forFn = function(arr, id,lev){
					for (var i = 0; i < arr.length; i++) {
						...
						if (item.fid==id) {
							item.lev=lev;
							// http://chat.cloud-cc.top/
							let host = 'http://chat.cloud-cc.top/'
							if (item.user.userpic.indexOf(host) == -1) {
								item.user.userpic = host + item.user.userpic
							}
							temp.push({
								id:item.id,
								fid:item.fid,
